...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  useEffect(() => {
    const handleResize = () => {
      setIsMobile(window.innerWidth < 312);
    };
  ...
  return isOpen ? (
    <div
      className={`${
        isOpen ? (isMobile ? "w-1/2" : "w-300") : "w-0"
      } transition-width duration-300 ease-in-out fixed top-0 border-l border-2 h-screen overflow-hidden`}
    >
      {true && (
        <button className="text-xl p-2 w-full text-left" onClick={toggleSidebar}>
          <span>{isOpen ? <HiX /> : <HiMenuAlt4 />}</span>
        </button>
      )}
    ...
    <div className="flex h-screen w-16 flex-col justify-between border-e bg-white">
      ...
      <div className="sticky inset-x-0 bottom-0 border-t border-gray-100 bg-white p-2">
        ...
        >
          ...
          <button className="absolute start-full top-1/2 ms-4 -translate-y-1/2 rounded bg-gray-900 px-2 py-1.5 text-xs font-medium text-white opacity-0 group-hover:opacity-100">
            Open
          </button>
        </button>
      </div>
